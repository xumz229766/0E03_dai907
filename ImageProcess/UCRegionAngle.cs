...
{
    public partial class UCRegionAngle : UserControl
    {
        HWindow hwin;
        RegionAngle ra = null;
        bool bInit = false;
        
        public UCRegionAngle(RegionAngle _r)
        {
            bInit = false;
            hwin = _r.hwin;
            ra = _r;
        ...
        private void setControls()
        {
            if (ra != null)
            {
                ...
                cbMeasureCircle.Checked = ra.bMeasureCircle;
                ...
                rbtnAngle.Checked = ra.bAngleTest;
                rbtnArea.Checked = !ra.bAngleTest;
                panel2.Enabled = !ra.bAngleTest;
        ...
        private void setRAValue()
        {
            if (ra != null)
            {
                ...
                ra.bMeasureCircle = cbMeasureCircle.Checked;
                ...
                ra.bAngleTest = rbtnAngle.Checked;
        ...
        public void test()
        {
            ...
            else if (ra.bMeasureCircle)
            {
                if (!ra.bMeasureResult)
                {
                    MessageBox.Show("未找到输出位置，请检查测量圆或去掉测量圆！");
                    return;
                }
                else
                {
        ...
        {
            panel2.Enabled = rbtnArea.Checked;
        }

        private void cbMeasureCircle_CheckedChanged(object sender, EventArgs e)
        {
            ...
            ra.GenRingContours();
            if (ra.bMeasureCircle)
            {
                cbModelCenter.Checked = false;
                if (!ra.bMeasureResult)
                {
                    MessageBox.Show("未找到输出位置，请检查测量圆或去掉测量圆！");
                    return;
